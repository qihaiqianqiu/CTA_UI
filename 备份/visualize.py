# Dependencies
import os, pdb, sys, numpy as np, pandas as pd
# 非交互模式防止内存泄露
import matplotlib
matplotlib.use('Agg') 
...
from calculate_parameter import client, trade_day
import utils
...
def get_pairwise_data(contract_pair:list, start_date:int, end_date:int):
    select_clause = 'SELECT contract, trading_date, time, ap1, av1, bp1, bv1 from cofund.ctp_future_tick '
...
# BarPlot
# BarPlot
def bar_plot_data_segmentation(contract_pair:list, start_date:int, end_date:int):
    # variable day is a bit ambigious - it is section indeed
    # 为包含夜盘数据，需要在数据库里往后检索一天
    save_date = end_date
    ...
    for date in range(start_date, trade_day[trade_day.index(end_date) + 1]):
        if date in trade_day:
            ...
            if len(pair_data) > 0:
                ...
                if len(df_s2) > 0:
                    df_section.append(df_s2)
                    label_day.append(x_label_s2)    

                if len(df_s0) > 0:
                    df_section.append(df_s0)
                    label_day.append(x_label_s0)

                if len(df_s1) > 0:
                    df_section.append(df_s1)
                    label_day.append(x_label_s1)
            
    counter = 0
    for section in df_section:
        # 买可得
        df_abr_b = (section['ap1_' + contract_pair[0]] - section['bp1_' + contract_pair[1]])
        # 卖可得
        df_abr_s = (section['bp1_' + contract_pair[0]] - section['ap1_' + contract_pair[1]])

        # 按Tick挂单量选取主动腿
        volume1 = section['av1_' + contract_pair[0]] + section['bv1_' + contract_pair[0]]
        v1sum = volume1.sum()
        volume2 = section['av1_' + contract_pair[1]] + section['bv1_' + contract_pair[1]]
        v2sum = volume2.sum()
        if v1sum > v2sum:
            # Second Instrument为主动腿
            df_abr_b_vol = section.apply(lambda x : x['bv1_' + contract_pair[1]], axis=1)
            df_abr_s_vol = section.apply(lambda x : x['av1_' + contract_pair[1]], axis=1)
            df_passive_vol = section.apply(lambda x: x['av1_' + contract_pair[0]] + x['bv1_' + contract_pair[0]], axis=1)
        else:
            # First Instrument为主动腿
            df_abr_b_vol = section.apply(lambda x : x['av1_' + contract_pair[0]], axis=1)
            df_abr_s_vol = section.apply(lambda x : x['bv1_' + contract_pair[0]], axis=1)
            df_passive_vol = section.apply(lambda x: x['av1_' + contract_pair[1]] + x['bv1_' + contract_pair[1]], axis=1)


        df_b = pd.concat([df_abr_b, df_abr_b_vol], axis=1, ignore_index=True)
        df_s = pd.concat([df_abr_s, df_abr_s_vol], axis=1, ignore_index=True)

        df_vol = pd.concat([df_b, df_s, df_passive_vol], axis=1, ignore_index=True)
        df_vol.columns = ['buyable', 'buy_vol', 'sellable', 'sell_vol', 'passive_vol']
        df_vol['buyable'] = df_vol['buyable'].astype('float')
        df_vol['sellable'] = df_vol['sellable'].astype('float')

        # 按被动腿与主动腿成交量的数量级差异决定被动腿对套利对成交的影响
        active_vol = (df_vol['buy_vol'] + df_vol['sell_vol']).mean()
        passive_vol = df_vol['passive_vol'].mean()
        ratio = passive_vol / active_vol
        if ratio > 10:
            # 成交由被动腿的挂单量决定
            df_vol['true_vol'] = df_vol['buy_vol'] + df_vol['sell_vol']
        else:
            # 成交量由主动腿被动腿共同决定
            df_vol['true_vol'] = (1 - 1/ratio) * df_vol['buy_vol'] + (1 - 1/ratio) * df_vol['sell_vol'] + 1 / ratio * df_vol['passive_vol']
        ...
        # 搜索可成价对应挂单量
        df_abr_vol = df_vol[(df_vol['buyable'] <= df_abr_mean) | (df_vol['sellable'] >= df_abr_mean)]
        # label rename & color set
        # Heatmap
        # volume relating to tradeable
        if len(df_abr) == 0:
            volume = 0
        else:
            volume = df_abr_vol['true_vol'].mean()
        vol_values.append(volume)
        bar_day.append(df_abr)

        # 存储每个单元的tick记录
        if not os.path.exists(os.path.join(r'Z:\300_Group\HFT\Program\CTA_UI\visualization_check', contract_pair[0] + '-' + contract_pair[1])):
            os.mkdir(os.path.join(r'Z:\300_Group\HFT\Program\CTA_UI\visualization_check', contract_pair[0] + '-' + contract_pair[1]))
        df_abr_vol.to_csv(os.path.join(os.path.join(r'Z:\300_Group\HFT\Program\CTA_UI\visualization_check', contract_pair[0] + '-' + contract_pair[1]), str(counter) + '.csv'), index=False)
        counter += 1

    #vol_values = [float(i)/sum(vol_values) for i in vol_values]
    # create a normalizer
    norm = Normalize(vmin=min(vol_values), vmax=max(vol_values))
    # normalize values
    norm_values = norm(vol_values)
    # choose a colormap
    cmap = cm.magma
    # create colors
    ...
    ax.set_title(contract_pair)
    ax.set_xticklabels(label_day)
    ax.title.set_size(120)
    
    ba = ax.boxplot(bar_day, patch_artist=True, showfliers=False)
    patches = ba["boxes"]
    ...
    plt.savefig(os.path.join(os.path.join(BATH_PATH, str(save_date)), str(save_date) + '-' + str(contract_pair[0]) + '-' + str(contract_pair[1]) + '.png'))
    fig.clf()
    plt.close()
    #plt.show()
...
def get_contract_lst():
    ...
    contract_pair_lst = [[utils.rename(x[0]), utils.rename(x[1])] for x in pairs]
    print(len(contract_pair_lst))
    
    return contract_pair_lst


def plot_single(date:int, contract_pair:list, back_period=30):
    F = open('./barplot/visualize_log.txt', 'w')
    start = trade_day[trade_day.index(date)-back_period]
    try:
        bar_plot_data_segmentation(contract_pair, start, date)

    except Exception as e:
        print(e)
        traceback.print_exception(etype=None, value=e, tb=e.__traceback__)
        print("Error:" , date, contract_pair)
    line = str(dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + ": " + str(contract_pair)
    F.write(line)
    F.write('\n')
    F.close()


def plot(date:int, back_period=30):
    contract_pair_lst = get_contract_lst()
    with Pool(5) as p:
        for i in range(0, len(contract_pair_lst), 4):
            contract_pairs = contract_pair_lst[i:i+4]
            for cp in contract_pairs:
                # 反套
                if cp[::-1] not in contract_pair_lst:
                    contract_pairs.append(cp[::-1])
            arg_lst = [(date, x, back_period) for x in contract_pairs]
            p.starmap(plot_single, arg_lst)
        p.close()
        p.join()
...
if __name__ == "__main__":
    ...
    yesterday = trade_day[trade_day.index(today)-1]
    if str(yesterday) not in os.listdir(BATH_PATH):
        os.mkdir(os.path.join(BATH_PATH, str(yesterday)))
    ...
    with Pool(5) as p:
        for i in range(0, len(contract_pair_lst), 4):
            ...
            for cp in contract_pairs:
                ...
                if cp[::-1] not in contract_pair_lst:
                    contract_pairs.append(cp[::-1])
            arg_lst = [(yesterday, x) for x in contract_pairs]
            p.starmap(plot, arg_lst)
        p.close()
        p.join()
